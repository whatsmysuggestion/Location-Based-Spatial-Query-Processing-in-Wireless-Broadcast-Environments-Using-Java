 ...
 * @author  Admin
 */
public class MobileHost1 extends javax.swing.JFrame {
	Connection con;
    Statement st;
    ServerSocket ss5,ss6,ss7;
	String port1;
    //Socket s1;
    Socket s2;
    Socket s3;
    ...
    public MobileHost1() {
		...
		try {
                ss5 = new ServerSocket(4444);

				ss6 = new ServerSocket(8181);

				ss7 = new ServerSocket(1111);
	...
	public void prtRecv()
	{
			while(true)
			{
				try	{
							port = ss5.accept();
							JOptionPane.showMessageDialog(this,"Port Received ");
	...
	public void prefinalres()
	{
			try
			{
						...
						DataInputStream displc = new DataInputStream(s6.getInputStream());
						getplc = displc.readUTF();
						String getplc1[] = getplc.split("&");
						for (int i = 0; i < getplc1.length; i++) 
							{
								String string = getplc1[i];
							}
                                 
                        jTextField3.setText(getplc1[3]);
                        jTextField4.setText(getplc1[1]);
                        jTextField5.setText(getplc1[2]);
...
public void rstRecv()
	{
			...
			{
						s5 = ss6.accept();
						DataInputStream disrst = new DataInputStream(s5.getInputStream());
...
// TODO add your handling code here:
    ...
    String cb2 = ""+jComboBox4.getSelectedItem();
 
    if(cb2.equals("Select"))
    {
         String cb3mess = "Plz Enter the Direction";
         JOptionPane.showMessageDialog(this,cb3mess);
    }
	else if(cb1.equals("Select"))
    {
         String cb2mess = "Plz Enter the Distance";
    ...
    else if(jButton4!=null)
    {
       ...
       if(b4==0)
       {
		...
		try {
					...
					dos.writeUTF(send);
					System.out.println("ss "+send+" "+cb2+" "+cb3);	
			  } catch (Exception e) {
					e.printStackTrace();
			}
    }
...
// TODO add your handling code here:
    ...
    try {
		...
		if(po.equals("2222"))
			...
			else if(po.equals("3333"))
				{
					s3 = new Socket("localhost",3333);
					String resSend3 = po+"&"+cb333+"&"+cb444+"&"+po1;
					DataOutputStream dousres3 = new DataOutputStream(s3.getOutputStream());
					dousres3.writeUTF(resSend3);
					JOptionPane jp=new JOptionPane();
					jp.showConfirmDialog(jp,"Do You Want Send Query To Nearest MobileHost3"); 
				}
		} catch (Exception e) {
			e.printStackTrace();
    }
...
double serverEndTime = ser+100000;//GEN-FIRST:event_jButton8ActionPerformed
// TODO add your handling code here:
...
// TODO add your handling code here:
String strfinal1 = jTextField3.getText();
String strfinal2 = jTextField4.getText();
String strfinal3 = jTextField5.getText();

try
	{
		if(strfinal1.equals("2222"))
			{
				String strfinal = d.retriveFinalrst("select Results from MH1 where Location='"+strfinal2+"' and Criteria='"+strfinal3+"' ");
				System.out.println("---------------->"+strfinal);
				s8 = new Socket("localhost",8282);
				DataOutputStream doutff2 =  new DataOutputStream(s8.getOutputStream());
				doutff2.writeUTF(strfinal);
				JOptionPane.showMessageDialog(this,"Response Sent To MH1");

			}else if(strfinal1.equals("3333"))
				{
					String strfinal = d.retriveFinalrst("select Results from MH1 where Location='"+strfinal2+"' and Criteria='"+strfinal3+"' ");
					System.out.println("---------------->"+strfinal);
					s8 = new Socket("localhost",8383);
					DataOutputStream doutff3 =  new DataOutputStream(s8.getOutputStream());
					doutff3.writeUTF(strfinal);
					JOptionPane.showMessageDialog(this,"Response Sent To MH3");
				}
	}catch (Exception er)	{
		er.printStackTrace();
		}
